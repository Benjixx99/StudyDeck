...
class CardWithSidesRepository(private val database: AppDatabase) {
    ...
    suspend fun insertCardWithTextSide(
        ...
        textSide: TextSideModel,
        cardSide: CardSide
    ): Pair<Long, Long> = dao.insertCardWithTextSide(card.toEntity(), textSide.toEntity(), cardSide, database)

    suspend fun insertTextSideAndUpdateCard(
        card: CardModel,
        textSide: TextSideModel,
        cardSide: CardSide
    ): Long = dao.insertTextSideAndUpdateCard(card.toEntity(), textSide.toEntity(), cardSide, database)

    suspend fun updateCardWithTextSide(
        card: CardModel,
        textSide: TextSideModel,
        cardSide: CardSide
    ) = dao.updateCardWithTextSide(card.toEntity(), textSide.toEntity(), cardSide, database)

    suspend fun insertCardWithAudioSide(
        card: CardModel,
        audioSide: AudioSideModel,
        cardSide: CardSide
    ): Pair<Long, Long> = dao.insertCardWithAudioSide(card.toEntity(), audioSide.toEntity(), cardSide, database)

    suspend fun insertAudioSideAndUpdateCard(
        card: CardModel,
        audioSide: AudioSideModel,
        cardSide: CardSide
    ): Long = dao.insertAudioSideAndUpdateCard(card.toEntity(), audioSide.toEntity(), cardSide, database)

    suspend fun updateCardWithAudioSide(
        card: CardModel,
        audioSide: AudioSideModel,
        cardSide: CardSide
    ) = dao.updateCardWithAudioSide(card.toEntity(), audioSide.toEntity(), cardSide, database)
}